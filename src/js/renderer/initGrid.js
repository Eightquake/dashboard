...
Packery.prototype.initShiftLayout = function(positions, attr) {
  ...
  if (typeof positions == "string") {
    try {
      ...
      global.problem.emit(
        "error",
        `Packery encountered a parse error: ${error}, maybe try clearing localstorage?`
      );
      this.layout();
      return;
    }
...
function initGrid() {
  return new Promise(function(resolve) {
    ...
    if (
      initPositions &&
      parsed.length < document.querySelectorAll(".grid-item").length
    ) {
